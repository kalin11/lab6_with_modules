...
public class VisitorImpl implements Visitor, Serializable {
    ...
    //todo вот это уже серверная часть должна быть
...
//    private HashSet<String> paths = new HashSet<>();
    ...
    @Override
    public Object visit(AddCommand c, Movie movie) {
...
//        );
    ...
    public LinkedList<Movie> visit(ClearCommand c) {
    ...
    //
    public Object visit(UpdateCommand c, Movie movie) {
        if (collection.isEmpty()) {
            return "коллекция пустая, чего обновлять?";
        } else {
            return collection.updateElement(movie.getId(), movie);

        }
    }

    public Object visit(RemoveCommand c) {
        if (collection.isEmpty()){
            return "коллекция пустая";
        }
        else {
            return collection.remove(Long.parseLong(c.getArgs()[0]));
        }
    }
    ...
    public Object visit(RemoveGreaterCommand c, Movie movie) {
        if (collection.isEmpty()) {
            return "коллекция пустая";
        } else {
            collection.removeG(movie);
            return collection;
        }
    }

    public Object visit(RemoveLowerCommand c, Movie movie) {
        if (collection.isEmpty()) {
            return "коллекция пустая";
        } else {
            collection.removeL(movie);
            return collection;
        }
    }
